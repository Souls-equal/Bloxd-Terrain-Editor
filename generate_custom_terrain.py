...
import time
import numpy as np
from bloxd_format import BloxdSchemWriter, _uvarint
...
name_to_id_path = os.path.join(script_dir, "uploads", "nameToId.json") if os.path.exists(os.path.join(script_dir, "uploads", "nameToId.json")) else os.path.join(script_dir, "nameToId.json")

try:
    with open(name_to_id_path, "r", encoding="utf-8") as fp:
...
biome_index = {b: i for i, b in enumerate(biome_names)}
...
def main():
    ...
    # Calcul de l'étendue verticale nécessaire selon MIN_HEIGHT et MAX_HEIGHT
    ...
    with open(OUTPUT_PATH, "wb") as f:
        ...
        for tx in range(tiles_x):
            ...
            for tz in range(tiles_z):
                ...
                # Attribution simple des biomes
                main_biome = np.zeros_like(Xi, dtype=np.int32)
                main_biome = np.where(topY <= SEA_Y + 3, biome_index["sand"], main_biome)
                main_biome = np.where((topY > SEA_Y + 3) & (topY < 95), biome_index["plain"], main_biome)
                main_biome = np.where(topY >= 95, biome_index["mountain"], main_biome)
